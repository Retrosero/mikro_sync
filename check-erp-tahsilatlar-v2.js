...
(async () => {
    try {
        console.log('Son 1 saat içinde eklenen tahsilat kayıtları kontrol ediliyor...\n');

        // Son 1 saat içinde eklenen CARI_HESAP_HAREKETLERI kayıtlarını getir
      ...
      AND cha_create_date > DATEADD(hour, -1, GETDATE())
      ORDER BY cha_RECno DESC
    `);

        console.log('=== SON CARI HESAP HAREKETLERİ (SON 1 SAAT) ===');
        if (cariHareketler.length === 0) {
            console.log("Son 1 saatte eklenen kayıt bulunamadı.");
        } else {
        ...
        console.log('\n\n=== SON ÖDEME EMİRLERİ (SON 1 SAAT) ===');
        // Son 1 saat içinde eklenen ODEME_EMIRLERI kayıtlarını getir
      ...
      AND sck_create_date IS NOT NULL
      ORDER BY sck_RECno DESC
    `);

        if (odemeEmirleri.length === 0) {
            console.log("Son 1 saatte eklenen kayıt bulunamadı veya create date null olmayan kayıt yok.");
        } else {
