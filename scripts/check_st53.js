...
async function checkST53() {
    try {
        console.log('ERP\'deki ST-53 kaydını kontrol ediyoruz...\n');
        ...
        const result = await mssqlService.query(`
            ...
            WHERE sth_evrakno_seri = 'ST' AND sth_evrakno_sira = 53
        `);

        if (result.length === 0) {
            console.log('❌ ST-53 kaydı bulunamadı!');
        } else {
            console.log('✅ ST-53 kaydı bulundu:\n');
            result.forEach((r, i) => {
