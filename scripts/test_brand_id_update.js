...
async function testBrandIdUpdate() {
    try {
        console.log('=== MARKA ID İLE GÜNCELLEME TEST ===\n');

        // SQLite'da bir marka seç
        sqliteService.connect(true);
        const testBrand = sqliteService.queryOne(`SELECT id, name FROM brand WHERE id = 429`);
        ...
        console.log('\nTest senaryosu:');
        console.log(`  Web'den gelen brand değeri: "${testBrand.id}" (sayı)`);
        console.log(`  Beklenen brand_id: ${testBrand.id}`);

        // sync_queue'ya test kaydı ekle - brand değeri olarak marka ID'si (sayı) gönder
        ...
        `, [
            {
                ...
                changes: {
                    brand: { old: currentProduct.brand_id, new: testBrand.id } // Marka ID'si (sayı) gönderiliyor
                },
        ...
        console.log(`  1. "${testBrand.id}" sayı olarak algılanacak`);
        console.log(`  2. Doğrudan brand_id=${testBrand.id} olarak yazılacak`);
